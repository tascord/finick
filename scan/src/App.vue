...
<template>
  <div class="w-screen h-[calc(100vh-10rem)] grid place-items-center">
    <main class="w-xl relative">
      <input autofocus type="text" @input="event => query((event.target! as HTMLInputElement).value)"
        class="w-full h-15 border border-gray-200 rounded-lg shadow-xl text-xl indent-4 outline-none bg-white"
        placeholder="Search">
      <ul v-if="items.length > 0"
        class="absolute mt-4 w-full border border-gray-200 rounded-lg shadow-sm max-h-[20rem] px-2 py-2.5 flex flex-col space-y-4 select-none overflow-y-auto bg-white">
